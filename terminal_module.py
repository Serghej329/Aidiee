...
class Terminal(QWidget):
    ...
    def init_ui(self):
        ...
        # Connect key press event for command history
        ...
        # Update styles and connect signals
    ...
    def handle_key_press(self, event):
        ...
        if event.key() == Qt.Key_Up:
            # Store current command if we're starting navigation
            if self.command_index == -1:
                self.current_command = self.input_field.text()
            
            if self.command_index < len(self.command_history) - 1:
                self.command_index += 1
                self.input_field.setText(self.command_history[self.command_index])
                
        elif event.key() == Qt.Key_Down:
            if self.command_index > -1:
                self.command_index -= 1
                if self.command_index == -1:
                    self.input_field.setText(self.current_command)
                else:
                    self.input_field.setText(self.command_history[self.command_index])
        else:
            # For all other keys, call the parent class's keyPressEvent
    ...
    def handle_command(self, command):
        ...
        if self.config['shell_enabled']:
                ...
                try:
                        ...
                        self.command_index = -1  # Reset command history navigation
                        
                        self.terminal_parser.execute_command(command)
                        
                        while True:
                                message = self.terminal_parser.get_output(timeout=0.1)
    ...
    def start_handle_command_thread(self):
        ...
        if command:
            command_cwd = self.current_cwd  # Store current CWD for this command
            with self.history_lock:
                command_group = {
                    "type": "command_group",
                    "command": command,
                    "cwd": command_cwd,  # Store CWD with command
                    "outputs": []
                }
